...
export function AttendancePanel() {
  ...
  return <div className="attendance-workspace">
    <section className="summary-chips attendance-summary">
      ...
      <div><span className="chip-icon violet"><Timer /></span><span><strong>{durationLabel(data.worked_minutes_today)}</strong><small>ساعات کار مؤثر امروز</small></span></div>
    </section>

    <section className="panel attendance-present-panel">
      <header className="panel-header"><div><h2>افراد حاضر</h2><p>حضور زنده؛ مراجعه موقت از ساعات کار مؤثر جدا است</p></div><Badge tone={present.length ? "success" : "neutral"}>{quantity(present.length)} نفر</Badge></header>
      {present.length ? <div className="attendance-present-list">{present.map((record) => <article key={record.id}><span className="attendance-avatar">{record.staff_member.name.charAt(0)}</span><div><strong>{record.staff_member.name}</strong>{record.is_temporary && <Badge tone="warning">موقت · بدون امتیاز</Badge>}<small>{staffPosition(record)}</small></div><span><small>زمان ورود</small><b>{dateTime(record.checked_in_at)}</b></span><span><small>مدت حضور</small><b>{durationLabel(record.duration_minutes)}</b></span><i /></article>)}</div> : <EmptyState icon={<DoorOpen />} title="در حال حاضر کسی ورود ثبت‌شده ندارد" text="پس از ثبت ورود همکاران، وضعیت زنده آن‌ها اینجا نمایش داده می‌شود." />}
    </section>
    ...
    <section className="panel attendance-history-panel">
      ...
      {records.length ? <div className="responsive-table"><table><thead><tr><th>پرسنل</th><th>ورود</th><th>خروج</th><th>چک‌لیست شروع / پایان</th><th>مدت حضور</th><th>وضعیت</th></tr></thead><tbody>{records.map((record) => { const entryCount = record.checklist_completions.filter((item) => item.phase === "entry").length; const exitCount = record.checklist_completions.filter((item) => item.phase === "exit").length; return <tr key={record.id}><td><div className="person-cell"><span>{record.staff_member.name.charAt(0)}</span><div><strong>{record.staff_member.name}</strong>{record.is_temporary && <Badge tone="warning">موقت · بدون امتیاز</Badge>}<small>{staffPosition(record)}</small></div></div></td><td><span className="attendance-time-cell check-in-time"><LogIn size={15} />{dateTime(record.checked_in_at)}</span></td><td>{record.checked_out_at ? <span className="attendance-time-cell check-out-time"><LogOut size={15} />{dateTime(record.checked_out_at)}</span> : <span className="attendance-open-label">هنوز حاضر است</span>}</td><td><span className="attendance-checklist-badges">{entryCount ? <Badge tone="success">شروع: {entryCount.toLocaleString("fa-IR")}</Badge> : <Badge>شروع: بدون مورد</Badge>}{exitCount ? <Badge tone="info">پایان: {exitCount.toLocaleString("fa-IR")}</Badge> : <Badge>{record.is_temporary ? "موقت: بدون چک‌لیست" : record.is_open ? "پایان: در انتظار" : "پایان: بدون مورد"}</Badge>}</span></td><td><strong>{durationLabel(record.duration_minutes)}</strong></td><td><Badge tone={record.is_open ? "success" : "neutral"}>{record.is_open ? "حاضر" : "خروج ثبت شد"}</Badge></td></tr>; })}</tbody></table></div> : <EmptyState icon={<UsersRound />} title="رویدادی با این فیلتر پیدا نشد" text="بازه زمانی، وضعیت یا عبارت جست‌وجو را تغییر دهید." />}
    </section>
  </div>;
}
