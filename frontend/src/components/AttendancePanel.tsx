...
export function AttendancePanel() {
  ...
  return <div className="attendance-workspace">
    ...
    <section className="panel attendance-history-panel">
      ...
      {records.length ? <div className="responsive-table"><table><thead><tr><th>پرسنل</th><th>ورود</th><th>خروج</th><th>مدت حضور</th><th>وضعیت</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td><div className="person-cell"><span>{record.staff_member.name.charAt(0)}</span><div><strong>{record.staff_member.name}</strong><small>{staffPosition(record)}</small></div></div></td><td><span className="attendance-time-cell check-in-time"><LogIn size={15} />{dateTime(record.checked_in_at)}</span></td><td>{record.checked_out_at ? <span className="attendance-time-cell check-out-time"><LogOut size={15} />{dateTime(record.checked_out_at)}</span> : <span className="attendance-open-label">هنوز حاضر است</span>}</td><td><strong>{durationLabel(record.duration_minutes)}</strong></td><td><Badge tone={record.is_open ? "success" : "neutral"}>{record.is_open ? "حاضر" : "خروج ثبت شد"}</Badge></td></tr>)}</tbody></table></div> : <EmptyState icon={<UsersRound />} title="رویدادی با این فیلتر پیدا نشد" text="بازه زمانی، وضعیت یا عبارت جست‌وجو را تغییر دهید." />}
    </section>
  </div>;
}
