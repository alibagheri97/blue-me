...
export default function MenuPage() {
  ...
  const visibleInPos = items.filter((item) => item.is_active).length;
  ...
  return <div className="page-stack menu-management-page">
    ...
    <section className="panel menu-catalogue">
      <div className="toolbar menu-toolbar"><label className="search-box"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="جست‌وجوی محصول منو…" /></label><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="">همه دسته‌بندی‌ها</option>{categories.data?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="all">همه وضعیت‌ها</option><option value="ready">آماده فروش</option><option value="draft">نیازمند اتصال</option><option value="inactive">پنهان از صندوق</option></select></div>
      {menu.isLoading ? <div className="center-loader"><Spinner /></div> : filtered.length ? <div className="menu-management-grid">{filtered.map((item) => {
        ...
        return <article key={item.id} className={!item.is_active ? "inactive" : ""}>
          ...
          <div className="menu-status-row"><Badge tone={item.recipe_configured ? "success" : "warning"}>{item.recipe_configured ? item.inventory_item_id ? "فروش مستقیم از انبار" : "مواد مصرفی تنظیم شده" : "نیازمند تعریف مواد مصرفی"}</Badge><Badge tone={!item.is_active ? "neutral" : item.is_available ? "info" : "danger"}>{!item.is_active ? "پنهان از صندوق" : item.is_available ? `در صندوق · قابل فروش${item.max_available_quantity !== null ? `: ${quantity(item.max_available_quantity)}` : ""}` : "در صندوق · ناموجود"}</Badge></div>
