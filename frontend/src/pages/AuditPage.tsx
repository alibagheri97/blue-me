...
const tone: Record<string,string> = { security: "warning", approvals: "info", inventory: "success", purchases: "success", orders: "info", users: "violet", kitchen: "warning", daily_needs: "warning", system: "neutral", menu: "violet", customers: "success", attendance: "success" };
...
function categoryLabel(value: string): string {
  return ({ security: "امنیت", approvals: "تأییدها", inventory: "انبار", purchases: "ورودی کالا", orders: "سفارش‌ها", users: "کاربران", kitchen: "آشپزخانه", daily_needs: "نیازهای خرید", system: "سامانه", menu: "منو", customers: "مشتریان", attendance: "حضور و غیاب" } as Record<string,string>)[value] || value;
}

function actionLabel(value: string): string {
  return ({ create: "ایجاد", update: "ویرایش", delete: "حذف", archive: "بایگانی", void: "ابطال", receive: "ورود کالا", adjust: "اصلاح موجودی", waste: "ضایعات", login: "ورود", login_failed: "ورود ناموفق", check_in: "ثبت ورود پرسنل", check_out: "ثبت خروج پرسنل", bootstrap: "راه‌اندازی اولیه", seed_catalog: "ساخت کاتالوگ اولیه", auto_create: "پیشنهاد خودکار خرید", auto_need_fulfilled: "تأمین خودکار", auto_need_cancelled: "لغو هشدار خودکار", status_change: "تغییر وضعیت", image_update: "تغییر تصویر", price_change_requested: "درخواست تغییر قیمت", price_changed: "تغییر قیمت", price_change_approved: "تأیید قیمت", price_change_rejected: "رد قیمت", daily_need_approved: "تأیید نیاز خرید", daily_need_rejected: "رد نیاز خرید" } as Record<string,string>)[value] || value.replaceAll("_"," ");
}

function entityLabel(value: string): string {
  return ({ user: "کاربر", session: "نشست", category: "دسته‌بندی", inventory_item: "کالای انبار", stock_movement: "گردش موجودی", purchase_receipt: "فاکتور ورودی", price_change_request: "درخواست قیمت", customer: "مشتری", menu_category: "دسته‌بندی منو", menu_item: "محصول منو", order: "سفارش", recipe: "دستور پخت", daily_need: "نیاز خرید", attendance_record: "رویداد حضور", catalog: "کاتالوگ" } as Record<string,string>)[value] || value;
}
