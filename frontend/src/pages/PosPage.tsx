...
export default function PosPage() {
  ...
  const [customerMode, setCustomerMode] = useState<"guest" | "existing" | "staff" | "new" | "waste">("guest");
  ...
  const isWaste = customerMode === "waste";
  const isInternal = isStaffMeal || isWaste;
  ...
  const submitOrder = () => {
    ...
    if (isWaste && orderNotes.trim().length < 3) { setError("دلیل اتلاف را با حداقل ۳ نویسه وارد کنید"); return; }
    const body: Record<string, unknown> = { is_system_waste: isWaste, items: lines.map((line) => ({ menu_item_id: line.item.id, quantity: line.quantity, notes: line.notes || null })), discount: isInternal ? 0 : Number(discount || 0), payment_method: isInternal ? "other" : payment, notes: orderNotes || null, order_type: orderType };
    ...
    <div className="page-stack pos-page">
      ...
      <div className="pos-layout">
        ...
        <aside className="panel cart-panel">
          ...
          <div className="customer-switch"><button className={customerMode === "guest" ? "active" : ""} onClick={() => { setCustomerMode("guest"); setCustomer(null); setStaffMember(null); }}>مهمان</button><button className={customerMode === "existing" ? "active" : ""} onClick={() => { setCustomerMode("existing"); setStaffMember(null); }}><Users size={15} /> مشتری</button><button className={customerMode === "staff" ? "active staff-mode" : ""} onClick={() => { setCustomerMode("staff"); setCustomer(null); setDiscount("0"); }}><ContactRound size={15} /> پرسنل</button><button className={customerMode === "new" ? "active" : ""} onClick={() => { setCustomerMode("new"); setCustomer(null); setStaffMember(null); }}><UserPlus size={15} /> جدید</button><button className={isWaste ? "active waste-mode" : ""} onClick={() => { setCustomerMode("waste"); setCustomer(null); setStaffMember(null); setDiscount("0"); }}><Trash2 size={15} /> اتلاف سیستم</button></div>
          ...
          {customerMode === "staff" && <div className="customer-picker staff-picker"><div className="staff-mode-note"><ContactRound size={17} /><span><strong>غذای پرسنلی</strong><small>مصرف ثبت می‌شود؛ احتساب هزینه در آمار با گزینه پرسنل است.</small></span></div><label className="search-box"><Search size={16} /><input value={staffSearch} onChange={(e) => setStaffSearch(e.target.value)} placeholder="جست‌وجوی نام یا سمت پرسنل…" /></label>{staffMember && <div className="selected-customer selected-staff"><span>{staffMember.name.charAt(0)}</span><div><strong>{staffMember.name}{staffMember.is_current_user && <em>خودم</em>}</strong><small>{staffPosition(staffMember)}</small></div><button onClick={() => setStaffMember(null)}><X size={16}/></button></div>}{!staffMember && staff.data?.map((person) => <button key={person.id} onClick={() => setStaffMember(person)}><span>{person.name.charAt(0)}</span><div><strong>{person.name}{person.is_current_user && <em>خودم</em>}</strong><small>{staffPosition(person)}{person.user ? ` · @${person.user.username}` : ""}</small></div></button>)}</div>}
          {isWaste && <div className="waste-mode-note"><Trash2 size={19} /><span><strong>اتلاف سیستم · بدون دریافت وجه</strong><small>مواد اولیه و بسته‌بندی انتخاب‌شده از انبار کسر می‌شود؛ هزینه در گزارش اتلاف و سود ثبت می‌شود. دلیل اتلاف الزامی است.</small></span></div>}
          ...
          <div className={`cart-options ${isStaffMeal ? "staff-cart-options" : ""}`}>{isInternal ? <div className="staff-no-payment"><BadgeCheck size={18} /><span><strong>بدون دریافت وجه</strong><small>{isWaste ? "هزینه اتلاف از سود کسر می‌شود؛ دریافتی صفر است." : "ارزش منو فقط در حساب داخلی پرسنل نگهداری می‌شود."}</small></span></div> : <><label><span>تخفیف</span><input value={discount} onChange={(e) => setDiscount(e.target.value)} type="number" min="0" step="0.01" /></label><label><span>روش پرداخت</span><select value={payment} onChange={(e) => setPayment(e.target.value)}><option value="card">کارت خوان</option><option value="cash">نقدی</option><option value="online">آنلاین</option><option value="other">سایر</option></select></label></>}<input value={orderNotes} onChange={(e) => setOrderNotes(e.target.value)} aria-label="توضیح کلی سفارش" maxLength={500} placeholder={isWaste ? "دلیل اتلاف (الزامی)" : isStaffMeal ? "توضیح غذای پرسنلی (اختیاری)" : "توضیح کلی سفارش (اختیاری)"} /></div>
          <div className={`cart-totals ${isStaffMeal ? "staff-cart-totals" : ""}`}><span><small>{isInternal ? "ارزش منو" : "جمع اقلام"}</small><strong>{money(subtotal)}</strong></span>{!isInternal && <span><small>تخفیف</small><strong>− {money(discount)}</strong></span>}<span className="grand-total"><small>{isInternal ? "قابل پرداخت" : "مبلغ نهایی"}</small><strong>{isInternal ? "بدون دریافت وجه" : money(total)}</strong></span></div>
          {error && <div className="form-error">{error}</div>}
          <Button className={`place-order ${isStaffMeal ? "staff-place-order" : ""}`} disabled={!lines.length || orderMutation.isPending || (isStaffMeal && !staffMember) || (isWaste && orderNotes.trim().length < 3)} onClick={submitOrder}>{orderMutation.isPending ? "در حال ثبت سفارش…" : isWaste ? <><Trash2 size={19} /> ثبت اتلاف سیستم</> : isStaffMeal ? <><ContactRound size={19} /> ثبت غذای پرسنلی · بدون دریافت</> : <><Check size={19} /> ثبت سفارش · {money(total)}</>}</Button>
        </aside>
      </div>

      <section className="panel orders-today"><header className="panel-header"><div><h2>سفارش‌های امروز</h2><p>هر سفارش را از ثبت تا تکمیل دنبال کنید.</p></div></header>{orders.data?.length ? <div className="order-board">{orders.data.map((order) => <button key={order.id} className={`order-ticket ${order.is_staff_meal ? "staff-order-ticket" : ""} ${order.order_type === "takeaway" ? "takeaway-order-ticket" : ""}`} onClick={() => setActiveOrder(order)}><div><strong>{order.order_number}</strong><span className="order-ticket-badges">{order.order_type === "takeaway" && <Badge tone="warning"><PackageOpen size={13} /> بیرون‌بر · {quantity(order.takeaway_package_count)} بسته</Badge>}{order.is_staff_meal && <Badge tone="violet">پرسنلی</Badge>}{order.is_system_waste && <Badge tone="danger">اتلاف سیستم</Badge>}<Badge tone={order.status === "cancelled" ? "danger" : order.status === "completed" ? "success" : "info"}>{statusLabel[order.status]}</Badge></span></div><h3>{order.customer_name === "Guest" ? "مهمان" : order.customer_name}</h3><p>{order.items.map((item) => `${quantity(item.quantity)}× ${item.name}`).join(" · ")}</p><footer><span>{dateTime(order.created_at)}</span><strong>{order.is_system_waste ? "اتلاف · بدون دریافت" : order.is_staff_meal ? "بدون دریافت" : money(order.total)}</strong></footer></button>)}</div> : <EmptyState icon={<ClipboardCheck />} title="امروز سفارشی ثبت نشده" text="سفارش‌های جدید به‌صورت زنده اینجا نمایش داده می‌شوند." />}</section>
...
function OrderCompleteModal({ order, printingMode, error, print, close }: { order: Order | null; printingMode: "customer" | "kitchen" | null; error: string; print: (mode: "customer" | "kitchen") => void; close: () => void }) {
  ...
  return <Modal open title="سفارش با موفقیت ثبت شد" onClose={close}>
    <div className="order-complete-step">
      ...
      <div className="order-complete-heading"><span>شماره سفارش</span><strong>{order.order_number}</strong><small>{order.customer_name === "Guest" ? "مهمان" : order.customer_name} · {order.is_staff_meal || order.is_system_waste ? "بدون دریافت وجه" : money(order.total)}</small>{order.order_type === "takeaway" && <Badge tone="warning"><PackageOpen size={14} /> بیرون‌بر · {quantity(order.takeaway_package_count)} بسته</Badge>}</div>
      <div className={`order-workflow-result ${sentToKitchen ? "sent-to-kitchen" : "completed-directly"}`}><ChefHat /><span><strong>{order.is_system_waste ? "اتلاف ثبت و موجودی کسر شد" : sentToKitchen ? "به آشپزخانه ارسال شد" : "سفارش تکمیل شد"}</strong><small>{order.is_system_waste ? "هزینه در حساب اتلاف ثبت شد؛ این ثبت به صف آماده‌سازی ارسال نمی‌شود." : sentToKitchen ? "حالت آشپزخانه روشن است و سفارش اکنون در صف آماده‌سازی قرار دارد." : "حالت آشپزخانه خاموش است؛ سفارش بدون مراحل سه‌گانه با موفقیت تکمیل شد."}</small></span></div>
      <div className="order-complete-actions">
        <Button variant="secondary" disabled={printingMode !== null} onClick={() => print("customer")}><Printer size={22} /><span><strong>{printingMode === "customer" ? "در حال آماده‌سازی…" : order.is_system_waste ? "چاپ برگه اتلاف" : order.is_staff_meal ? "چاپ برگه پرسنلی" : "چاپ رسید مشتری"}</strong><small>چاپ مستقیم، بدون اسکرول</small></span></Button>
        {!order.is_system_waste && <Button variant="secondary" disabled={printingMode !== null} onClick={() => print("kitchen")}><ChefHat size={22} /><span><strong>{printingMode === "kitchen" ? "در حال آماده‌سازی…" : "چاپ فیش آشپزخانه"}</strong><small>نسخه خوانا برای آماده‌سازی</small></span></Button>}
      </div>
      {error && <div className="form-error">{error}</div>}
...
function OrderModal({ order, close, print, changeStatus, edit, remove }: { order: Order | null; close: () => void; print: (mode: "customer" | "kitchen") => void; changeStatus: (status: string) => void; edit: () => void; remove: () => void }) {
  ...
  return <Modal open title={order.order_number} onClose={close}><div className="order-detail-head"><div><small>{order.is_system_waste ? "حساب اتلاف" : order.is_staff_meal ? "پرسنل" : "مشتری"}</small><strong>{order.customer_name === "Guest" ? "مهمان" : order.customer_name}</strong><span>{dateTime(order.created_at)}</span></div><div className="order-detail-badges">{order.order_type === "takeaway" && <Badge tone="warning"><PackageOpen size={13} /> بیرون‌بر · {quantity(order.takeaway_package_count)} بسته</Badge>}{order.is_staff_meal && <Badge tone="violet">غذای پرسنلی</Badge>}{order.is_system_waste && <Badge tone="danger">اتلاف سیستم</Badge>}<Badge tone={order.status === "cancelled" ? "danger" : order.status === "completed" ? "success" : "info"}>{statusLabel[order.status]}</Badge></div></div><div className="order-detail-lines">{order.items.map((line) => <div key={line.id}><span>{quantity(line.quantity)} × {line.name}{line.notes && <small>{line.notes}</small>}</span><strong>{order.is_system_waste ? money(line.line_cost) : order.is_staff_meal ? quantity(line.quantity) : money(line.line_total)}</strong></div>)}</div><div className={`order-detail-total ${order.is_staff_meal ? "staff-order-total" : ""}`}><span>{order.is_staff_meal || order.is_system_waste ? "نوع ثبت" : "مبلغ نهایی"}</span><strong>{order.is_system_waste ? "اتلاف سیستم · بدون دریافت وجه" : order.is_staff_meal ? "مصرف داخلی · بدون دریافت وجه" : money(order.total)}</strong></div>{order.status !== "cancelled" && <Button variant="secondary" className="full-button order-edit-trigger" onClick={edit}><Pencil size={17} /> ویرایش اقلام و اطلاعات سفارش</Button>}<div className="receipt-actions">{!order.is_system_waste && <Button variant="secondary" onClick={() => print("kitchen")}><ChefHat size={17} /> فیش آشپزخانه</Button>}<Button variant="secondary" onClick={() => print("customer")}><Printer size={17} /> {order.is_system_waste ? "برگه اتلاف" : order.is_staff_meal ? "برگه پرسنلی" : "رسید مشتری"}</Button></div>{next[order.status] && <Button className="full-button" onClick={() => changeStatus(next[order.status]!.value)}>{next[order.status]!.label}</Button>}<Button variant="danger" className="full-button order-delete-trigger" onClick={remove}><Trash2 size={17} /> حذف کنترل‌شده سفارش</Button></Modal>;
}
...
function ReceiptContent({ receipt, brand }: { receipt: ReceiptState; brand: { business_name: string; logo_url: string | null } }) {
  ...
  return <div className={`receipt receipt-mode-${mode}`} id="printable-receipt" dir="rtl">
      <header className="receipt-header">
        ...
        <span className="receipt-copy-label">{order.is_system_waste ? "ثبت هزینه اتلاف" : mode === "kitchen" ? "فیش آماده‌سازی" : order.is_staff_meal ? "مصرف داخلی" : "شاورماچی"}</span>
        <h2>{order.is_system_waste ? "برگه اتلاف سیستم" : mode === "kitchen" ? "آشپزخانه" : order.is_staff_meal ? "غذای پرسنلی" : brand.business_name}</h2>
      ...
      <div className="receipt-customer"><span>{order.is_system_waste ? "حساب اتلاف" : order.is_staff_meal ? "پرسنل" : "مشتری"}</span><strong>{order.customer_name === "Guest" ? "مهمان" : order.customer_name}</strong></div>
      ...
      {order.is_system_waste && <div className="staff-receipt-note"><strong>اتلاف سیستم · بدون دریافت وجه</strong><span>هزینه از سود کسر شده است</span><strong>دلیل: {order.notes}</strong></div>}
      {mode === "customer" && order.is_staff_meal && <div className="staff-receipt-note"><strong>بدون دریافت وجه</strong><span>مصرف داخلی پرسنل · بدون دریافت وجه</span></div>}
      ...
      <footer><strong>{mode === "customer" ? document.customer_copy.footer : "با دقت آماده و کنترل شود"}</strong><span>{mode === "customer" ? order.is_system_waste ? "ثبت‌شده در حساب اتلاف" : order.is_staff_meal ? "ثبت‌شده در حساب پرسنل" : "به امید دیدار دوباره" : "کنترل نهایی پیش از تحویل"}</span><small>{order.order_number}</small></footer>
    </div>;
}
