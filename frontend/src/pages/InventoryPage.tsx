...
import { AlertTriangle, ArrowDownToLine, Boxes, Camera, Check, ChevronRight, CircleDollarSign, Layers3, MoreHorizontal, PackageOpen, Plus, Search, SlidersHorizontal, X } from "lucide-react";
...
export default function InventoryPage() {
  ...
  const saveItem = (event: FormEvent<HTMLFormElement>) => {
    ...
    const body: Record<string, unknown> = {
      ...
      unit: form.get("unit"), reorder_level: form.get("reorder_level"), description: form.get("description") || null,
    };
      ...
      {tab === "stock" ? <>
        ...
        <section className="panel catalogue-panel">
          ...
          {items.isLoading ? <div className="center-loader"><Spinner /></div> : items.data?.items.length ? <div className="inventory-grid">{items.data.items.map((item) => { const isLow = Number(item.current_quantity) <= Number(item.reorder_level); return <article className="inventory-card" key={item.id}>
            <button className="item-image" onClick={() => setDetailItem(item)}>{assetUrl(item.image_path) ? <img src={assetUrl(item.image_path)} alt="" /> : <PackageOpen size={30} />}{isLow && <Badge tone="danger">موجودی کم</Badge>}</button>
...
function CategoryManager({ open, close, categories }: { open: boolean; close: () => void; categories: Category[] }) {
  const client = useQueryClient();
  const [error, setError] = useState("");
  const mutation = useMutation({ mutationFn: (body: object) => api("/inventory/categories", { method: "POST", body }), onSuccess: () => { client.invalidateQueries({ queryKey: ["categories"] }); setError(""); }, onError: (reason) => setError(reason instanceof ApiError ? reason.message : "ایجاد دسته‌بندی انجام نشد") });
  useEffect(() => { if (!open) setError(""); }, [open]);
  const submit = (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const form = new FormData(e.currentTarget); mutation.mutate({ name: form.get("name"), description: form.get("description") || null, color: form.get("color") }); e.currentTarget.reset(); };
  return <Modal open={open} title="دسته‌بندی‌های انبار" onClose={close}><div className="category-list">{categories.map((category) => <div key={category.id}><span style={{ background: category.color }} /><div><strong>{category.name}</strong><small>{category.description || "بدون توضیح"}</small></div></div>)}</div><form className="inline-category-form" onSubmit={submit}><label className="field"><span>دسته‌بندی جدید</span><input name="name" required placeholder="نام دسته‌بندی" /></label><label className="field color-field"><span>رنگ</span><input name="color" type="color" defaultValue="#2563eb" /></label><label className="field field-wide"><span>توضیحات</span><input name="description" placeholder="اختیاری" /></label>{error && <div className="form-error field-wide">{error}</div>}<Button type="submit" disabled={mutation.isPending}><Plus size={17} /> افزودن دسته</Button></form></Modal>;
}
