      ...
      <Modal open={itemForm !== null} title={itemForm === "new" ? "ایجاد کالای انبار" : "ویرایش کالای انبار"} onClose={() => setItemForm(null)} wide>
        <form className="form-grid" onSubmit={saveItem}>
          ...
          <section className="pricing-editor simple-pricing-editor field-wide">
            ...
            <div className="pricing-note"><CircleDollarSign size={18} /><span>{user?.role !== "root" ? "اطلاعات کالا فوراً ذخیره می‌شود؛ تغییر قیمت خرید و فروش پس از تأیید مدیر کل اعمال خواهد شد." : "تغییر قیمت خرید، موجودی فعلی را ارزش‌گذاری مجدد می‌کند و روی سفارش‌های آینده اثر می‌گذارد؛ سوابق گذشته تغییر نمی‌کنند."}</span></div>
          </section>
          ...
          <div className="form-actions field-wide"><Button type="button" variant="secondary" onClick={() => setItemForm(null)}>انصراف</Button><Button type="submit" disabled={mutation.isPending}>{mutation.isPending ? "در حال ذخیره…" : user?.role !== "root" && itemForm !== "new" ? "ذخیره و ارسال قیمت‌ها" : "ذخیره کالا"}</Button></div>
        </form>
      </Modal>
